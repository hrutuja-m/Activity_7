...
//4//
    function brushStart() {
        d3.selectAll("circle").classed("selected", false); // Clear previous selections
    }
    ...
    legends_items.append("rect")
        ...
        legends_items.on("click", (event, d) => {
            const isActive = d3.select(event.currentTarget).classed("legendSelect");
        
            // Reset all circles
            d3.selectAll("circle").classed("dimmed", true);
        
            if (!isActive) {
                // Highlight points of the selected category
                d3.selectAll(`circle.${d}`).classed("dimmed", false).classed("selected", true);
            } else {
                // Reset when legend is deselected
                d3.selectAll("circle").classed("dimmed", false).classed("selected", false);
            }
        
            // Toggle legend highlight
            d3.select(event.currentTarget).classed("legendSelect", !isActive);
        });
